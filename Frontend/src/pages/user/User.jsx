...
export default function AppointmentBooking() {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        const res = await axios.get(`/api/companies/by-subdomain/${subdomain}`);
        const companyId = res.data.data.company_id;
        setCompanies(companyId);

        // Now fetch services
        // const servicesRes = await axios.get(`/api/services/${companyId}`);
        // setServices(servicesRes.data.data);

        // Optionally set company customization
        // setCompany(res.data.data);
      } catch (err) {
        console.error("Invalid subdomain or company not found", err);
      }
    };
          ...
          {currentStep === 1 && (
            <div className="p-8 animate-slide-in-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label htmlFor="company_id" className="text-sm font-medium" style={styles.text}>
                    Select Company *
                  </label>
                  <select
                    id="company_id"
            ...
            <div className="p-8 animate-slide-in-up">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  {!formData.company_id && (
                    <p className="text-sm mt-1" style={styles.text}>Please select a company first</p>
                  )}
